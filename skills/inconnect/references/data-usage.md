...
inconnect data-usage account-export --oid <oid>              # 导出账号用量 Excel
inconnect data-usage router-export --oid <oid>               # 导出路由器用量 Excel
```
...
| 需求 | 用 |
|---|---|
| VPN 隧道承载的用量（按账号/路由器/组织） | `inconnect data-usage *` |
| 单台设备的运营商口流量（按日/按月，site 侧） | `inconnect router traffic-day <id>` |
