...
inconnect firmware get <id> --oid <oid>                      # 包详情
...
inconnect firmware devices <id> list --oid <oid>             # 升级任务中的设备
inconnect firmware devices <id> add ... --oid <oid>          # 添加设备到升级任务
inconnect firmware devices <id> remove ... --oid <oid>       # 移除设备
...
1. `inconnect router online-trend <id>` 看是否曾上线又掉线
2. `inconnect router signal <id>` 看信号是否足以完成下载
3. `inconnect firmware job-stats <id>` 看该设备的最终状态
