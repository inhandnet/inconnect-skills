...
```bash
inconnect router running-config <id> --oid <oid>        # 实时（在线时优先用这个判断现状）
...
```bash
inconnect router device-config set <id> --content-file cfg.txt --oid <oid>
```
...
```bash
inconnect router config-send <id> --oid <oid>
```
...
```bash
inconnect drc list --oid <oid>
inconnect drc create --name <n> --content <...> --oid <oid>
inconnect drc devices <id> add ... --oid <oid>
```
