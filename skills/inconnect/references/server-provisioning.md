...
```bash
inconnect server list --oid <oid>
inconnect server get <id> --oid <oid>
...
```bash
inconnect server create ... --oid <oid>            # 仅 DB 记录
inconnect server create ... --deploy --oid <oid>   # 创建并在 K8s 部署 OpenVPN Pod
inconnect server deploy <id> --oid <oid>           # 对已有记录部署/重新部署
```
...
```bash
inconnect server stop --oid <oid>      # 停止该组织的服务器（VPN 全断！）
inconnect server recover --oid <oid>   # 恢复
```
...
inconnect server command --oid <oid> ...      # 向组织服务器发送命令
```
...
`inconnect router transfer <id> --to <target-oid> --oid <src-oid>` 有严格前置：
...
执行前先 `inconnect router transfer --help` 看完整参数，逐条确认前置条件，再谨慎操作。
