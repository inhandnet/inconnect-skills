...
inconnect router ssh <id> --server <ngrok:port> --oid <oid>  # 经 ngrok 隧道开 SSH（拉日志/进 shell，见 ssh-access.md）
inconnect router tunnel-close <uuid>                         # 关闭 web/ssh 隧道（拉完日志即关，省 keepalive）
# 注意：router exec 在设备端未实现（mqttagent 无处理器，恒返回 "handle ... failed"）。
#       要在设备上跑命令，唯一可用路径是 router ssh，见 references/ssh-access.md
...
`router list` / `router get` 返回两个**相互独立**的状态字段，含义不同，**极易混淆**：

| 字段        | 含义                          | 说明                                                              |
| ----------- | ----------------------------- | ----------------------------------------------------------------- |
| `online`    | 设备管理通道（MQTT）是否在线  | `1`/`0`。平台能否管到设备：配置下发、远程控制、ngrok、reboot 全靠它 |
| `connected` | VPN 隧道（OpenVPN）是否已建立 | `true`/`false`。设备是否真正挂上 VPN、能否跑 VPN 流量              |

> **两者独立**：`online=1, connected=false` 很常见——设备能管，但没建 VPN 隧道（如所属服务器未运行）。
> `router stats` 与 `router list` 的 `stats` 字段里，online/offline 与 connected/disconnected 是**两套独立计数**。
> `online` 可能因上报滞后而不准；要确认设备是否真能管，结合诊断命令实测（见 `references/diagnostics.md`）。
...
| 登设备拉日志/SSH 进设备/进 root shell/show log/平台数据不够要进设备 | `references/ssh-access.md` |
