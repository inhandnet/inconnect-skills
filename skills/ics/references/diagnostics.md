...
```bash
ics router get <id> --oid <oid> --verbose 100
```
...
```bash
ics router online-trend <id> --after <start> --before <end> --oid <oid>
```
...
ics router running-config <id> --oid <oid>        # 设备实时配置（需在线）
```
...
```bash
ics router list --online false --oid <oid>              # 所有离线路由器
ics router list --online false --query IR900 --oid <oid> # 按名称/SN 过滤
ics router stats --oid <oid>                             # 在线/离线计数 + 按型号分布
```
